...
from Get_data_Onet import get_test_data, get_train_data
import argparse
...
def main(args):
    ...
    X_train = get_train_data(data_file=args.data_file, device=device)

    model_type = 'ONet'
    width = 100
    model_1 = DeepONet(sensor_dim=100, h_dim=width, in_dim=1, actv=nn.ReLU()).to(device)
    ...
    for i in range(args.epoch):
        ...
        loss = 100 * nn.MSELoss()(model_1(input), input[-1])

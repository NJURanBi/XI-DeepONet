...
def optimize_parameters_adam(lr, epochs, print_epoch, train_xop, train_xon, train_xb, train_xif, batch, model, test_data, device):
    def loss_omega_p(data):
        ...
        dphi2_dx2 = 2.
        dphi2_dy2 = 2.
        Dphi2_dx2 = du2_dx2 + du2_dxdz * dphi_dx + du2_dzdx * dphi_dx + du2_dz2 * dphi_dx * dphi_dx + du_dz * dphi2_dx2
        Dphi2_dy2 = du2_dy2 + du2_dydz * dphi_dy + du2_dzdy * dphi_dy + du2_dz2 * dphi_dy * dphi_dy + du_dz * dphi2_dy2
        loss = torch.mean((Dphi2_dx2 + Dphi2_dy2 - data[6]) ** 2)
        return loss
    ...
    def loss_interface(data):
        ...
        dphi_dy = data[5]
        nor = data[-1]
        dUp_dn = torch.sum((torch.cat((du_dx + du_dz * dphi_dx, du_dy + du_dz * dphi_dy), 1) * nor), 1)
        dUn_dn = torch.sum((torch.cat((du_dx - du_dz * dphi_dx, du_dy - du_dz * dphi_dy), 1) * nor), 1)
        loss = torch.mean((10 * dUp_dn - dUn_dn) ** 2)
        return loss

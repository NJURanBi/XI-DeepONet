...
class XIDeepONet(nn.Module):
    def __init__(self, sensor_dim1, sensor_dim2, h_dim, in_dim, actv = nn.Tanh()):
        super(XIDeepONet, self).__init__()
        self.actv1 = actv
        #self.actv2 = nn.ReLU()
        self.actv2 = actv

        self.b_phi_linear_input = nn.Linear(sensor_dim1, h_dim)
        ...
        self.b_f_linear_input = nn.Linear(sensor_dim1, h_dim)
        ...
        self.b_b_linear_input = nn.Linear(sensor_dim2, h_dim)
        self.b_b_linear1 = nn.Linear(h_dim, h_dim)
        self.b_b_linear2 = nn.Linear(h_dim, h_dim)
        self.b_b_linear3 = nn.Linear(h_dim, h_dim)
        self.b_b_linear4 = nn.Linear(h_dim, h_dim)
    ...
    def forward(self, phi_tr, f_tr, b_tr, x_tr):
        ...
        b_tr = b_tr
        out = self.actv2(self.b_b_linear_input(b_tr))
        out = self.actv2(self.b_b_linear1(out))
        out = self.actv2(self.b_b_linear2(out))
        out = self.actv2(self.b_b_linear3(out))
        branch_b = self.actv2(self.b_b_linear4(out))
        ...
        output = torch.sum(branch_f * branch_b * branch_phi * trunk, dim=-1, keepdim=True) + self.p['bias']

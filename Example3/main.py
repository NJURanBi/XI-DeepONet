...
def main(paras):
    model = XIDeepONet(sensor_dim1=100, sensor_dim2=36, h_dim=paras.width, in_dim=3, actv=nn.Tanh()).to(device).float()
    train_xop, train_xon, train_xb, train_xif = generate_train_data(paras.sample_num, device)
    test_data = generate_test_data(10000, device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch', type=list, default=[5000, 2000, 2000])
